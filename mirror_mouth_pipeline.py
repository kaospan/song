...
LYRICS_FILE = normalize_env_path(os.environ.get("LYRICS_FILE")) or "mirror_mouth_lyrics.txt"
...
QUEUED_STATUS_SLEEP_SECONDS = float(os.environ.get("QUEUED_STATUS_SLEEP_SECONDS", "10"))
...
def _prompt_cache_fingerprint(
    ...
    payload = {
        ...
        "generic_config_overrides": generic_config_overrides or {},
    }
...
def _apply_generic_config_overrides(base_config, overrides):
    if not overrides or not isinstance(overrides, dict):
        return base_config

    data = dataclasses.asdict(base_config)
    allowed = {
        "narrative_mode",
        "style_profile",
        "camera_mode",
    ...
    if "script_max_lines_per_segment" in data:
        try:
            data["script_max_lines_per_segment"] = max(1, int(data["script_max_lines_per_segment"]))
        except (TypeError, ValueError):
            data["script_max_lines_per_segment"] = base_config.script_max_lines_per_segment

    if "export_prompt_bundle" in data:
        data["export_prompt_bundle"] = bool(data["export_prompt_bundle"])

    return GenericVideoPipelineConfig(**data)
...
def build_or_load_prompt_assets(
    *,
    ...
    generic_config = _apply_generic_config_overrides(generic_config, generic_config_overrides)
...
def run_pipeline(
    ...
    lyrics_max_lines_per_segment=LYRICS_MAX_LINES_PER_SEGMENT,
    segment_prompt_template_override=None,
    generic_config_overrides=None,
    ...
    try:
        ...
        segment_prompt_template = (
            (segment_prompt_template_override or "").strip() or default_segment_prompt_template
        )
